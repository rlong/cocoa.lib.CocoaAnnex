...
@implementation CAOutputStreamHelper
...
+(void)writeTo:(NSOutputStream*)outputStream buffer:(const uint8_t *)buffer bufferLength:(NSUInteger)bufferLength {
    ...
    while( 0 < bufferRemaining ) {
        ...
        if( 0 > numBytesWritten ) {
            ...
            CABaseException* e = [CABaseException baseExceptionWithOriginator:self line:__LINE__ callTo:@"[NSOutputStream write:maxLength:]" failedWithError:error];
