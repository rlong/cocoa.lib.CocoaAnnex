...
@implementation CAInputStreamHelper
...
+(void)writeFrom:(NSInputStream*)inputStream toOutputStream:(NSOutputStream*)outputStream count:(long long)count {
    ...
    while( 0 < bytesRemaining ) {
        ...
        if( nil != [inputStream streamError] ) {
            @throw [CABaseException baseExceptionWithOriginator:self line:__LINE__ callTo:@"[NSInputStream read:maxLength]" failedWithError:[inputStream streamError]];
        }
...
+(void)writeFrom:(NSInputStream*)inputStream toFileDescriptor:(int)fd count:(long long)count {
    ...
    while( 0 < bytesRemaining ) {
        ...
        if( -1 == bytesWritten ) {
            ...
            if( EPIPE == errno ) {
                [e setErrorDomain:[CAStreamHelper ERROR_DOMAIN_BROKEN_PIPE]];
            }
