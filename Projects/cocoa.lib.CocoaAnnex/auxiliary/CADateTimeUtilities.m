...
@implementation CADateTimeUtilities
...
+(void)getTime:(struct timeval *)tp {
	[CADateTimeUtilities getTime:tp inTimezone:NULL];
}
...
+(long)getElapsedTimeSince:(struct timeval*)timestamp {
	struct timeval now;
	[CADateTimeUtilities getTime:&now];
	long answer = [CADateTimeUtilities getTimeFrom:timestamp to:&now];	
	return answer;
}
